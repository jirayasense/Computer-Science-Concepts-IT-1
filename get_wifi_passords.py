...
show_profiles_output = subprocess.run([*show_cmd, "profiles"], capture_output=True).stdout.decode()
...
if wifi_profile_names:
    for name in wifi_profile_names:
        ...
        profile_info = subprocess.run([*show_cmd, "profile", name], capture_output=True).stdout.decode()
        ...
        if re.search(re_pattern_passwordExistence, profile_info):
            ...
            wifi_profile_pass_info = subprocess.run([*show_cmd, "profile", name, "key=clear"], capture_output=True).stdout.decode()
